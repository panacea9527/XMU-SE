...
Vector3f RayColor(const Ray& ray, Scene& scene, int depth=0, bool test=false)
{
	...
	if (closest_hp.objIdx != -1)
	{
		...
		bool isShadow = false;
		// shadow ray 求交
		Vector3f shadowDir = (lightpos - closest_hp.position).normalized();
		Ray shadow_ray(closest_hp.position + 1e-2 * shadowDir, shadowDir);

		// 检查 shadow ray 是否被场景中的物体遮挡
		for (int i = 0; i < scene.ObjectCount(); ++i)
		{
			HitInfo ht;
			if (scene.GetObjectPtr(i)->Hit(shadow_ray, ht))
			{
				if (ht.t > 0 && ht.t < (lightpos - closest_hp.position).norm())
				{
					isShadow = true;
					break;
				}
