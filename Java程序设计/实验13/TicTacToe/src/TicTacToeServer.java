...
public class TicTacToeServer extends JFrame 
{
   ...
   */
   ...
   // place code in this method to determine whether game over
// determine whether game over and return the winner's mark if there is a winner
   public String isGameOver()
   {
      for (int i = 0; i < 3; i++) {
         if (!board[i * 3].equals("") &&
                 board[i * 3].equals(board[i * 3 + 1]) &&
   ...
   private class Player implements Runnable 
   {
      ...
      public void run()
      {
         ...
         try 
         {
            ...
            while ((gameOverResult = isGameOver()) == null) {
         ...
         finally
         {
            try
            {
               if (gameOverResult.equals("Draw")) {
                  output.format("Game over. It's a draw.\n");
                  output.flush();
                  players[(playerNumber + 1) % 2].output.format("Game over. It's a draw.\n");
                  players[(playerNumber + 1) % 2].output.flush();
               } else {
                  if (gameOverResult.equals(mark)) {
                     output.format("Game over. You win!\n");
                     output.flush();
                     players[(playerNumber + 1) % 2].output.format("Game over. You lose.\n");
                     players[(playerNumber + 1) % 2].output.flush();
                  } else {
                     output.format("Game over. You lose.\n");
                     output.flush();
                     players[(playerNumber + 1) % 2].output.format("Game over. You win!\n");
                     players[(playerNumber + 1) % 2].output.flush();
                  }
               }
